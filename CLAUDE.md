...
- 战斗人物朝向由当前目标的实际舞台坐标逐 token 决定，不再按阵营写死；目标变化或背后受击时，人物会在当前攻击定格结束后以脚底为轴转身 180ms。背刺先向前踉跄再回身，弓兵被近身会转向来敌并停射，突破到战线前方的骑兵会回头面对身后步兵；这些均为表现层，不改变事件拍点或结算。
- 精英匪队身份以 `battleScript.eliteEnemy` 纯表现元数据进入舞台：标题为「精锐匪队」，全队使用更重的披肩剪影，首 token 另带狼尾旗；兵种、公式与伤亡不变。战斗脚本的既定伤亡优先落在仍存活的近战屏障，弓兵最后才成为目标；匪徒首次攻击后排弓兵前必须按实测距离走到 24px 近战范围，追击事件先于命中拍。
...
- 全屏战斗舞台：`.battle-stage` 为 `fixed; inset:0` 且显式 `100vw/100vh` 后接 `100dvw/100dvh`（地址栏不属于网页，不用 Fullscreen API），背景改为不透明 `--paper`——原先 `rgba(28,25,22,.9)` 的 90% 遮罩正是玩家看到的「四周暗化地图」。`.stage-paper` 由 `min(1100px)×min(44vh,400px)` 居中卡片改为 full-bleed（无宽高上限、无 `box-shadow`、无居中与外边距），安全区改挂在 `.stage-plates`/`.stage-log`/`.stage-hint` 上，纸面本身仍出血。战斗中 `#app-overlay` 提到 z-index 6（它是 `z-index:2` 的堆叠上下文，HUD 的 5 永远打不过舞台的 4——旧注释「舞台在 HUD 之下」是错的），HUD 只保留唯一的 `#sound-button` 置于左上安全区，倍速 chip 在右上安全区，暂停/设置/统计全部隐藏，长按跳过不变。
- 战场随页面填充而非等比放大：图形高度仍受 `TOKEN_MAX_HEIGHT_PX=104` 封顶，改为让**纵深**随舞台增长（`CAMERA.DEPTH_RISE_RATIO=0.5`、`DEPTH_RISE_MAX_PX=480`、`depthRiseFor()`，短横屏保留原 140px 下限）；`layoutGround()` 用实测世界高与band高把中线置于 `STAGE_BAND_CENTRE_FROM_BOTTOM=0.46`，写入 `--ground-bottom`/`--wall-bottom`，`.stage-ranks`/`.stage-wall`/`.stage-archers` 一起就位。阵型间距是冻结规则，故内容宽度由 `--field-w: min(100%,1280px)` 约束并与 plates/log 共用一列——超宽桌面两侧是宣纸留白而非卡片。弓兵的后撤位（`ARCHER_REAR_OFFSET_PX`）、被越阵退步与 melee 抖动统一走 `outwardRoom()`/`clampMeleeOffset()` 容器夹取，窄屏不再把人推出纸外。
...
- 当前验收基线：demo 19/19 套件、full 18/18 套件（新增 `test_battle_fullscreen` 12/12：视口占满与 dvh 回退、不透明无地图透出、纸面无卡片、地图面与 HUD 抑制且声/静唯一可用、双角控件在安全区内、地面线由实测 band 决定、四种视口 band 居中且填充 ≥45%、图形不被放大、dispose 清场）（新增 `test_audio_music` 29/29：FakeAudioContext 覆盖五状态、乐句必有铺底与声音时间轴无缝、密度呼吸、主题陈述/变体/回收、古风音色与三声部、等功率交叉淡与不叠实例、声/静按键唯一且标签随引擎实时状态、静音后音乐与 SFX 同归零并持久化、静音启动不开 AudioContext、缺字段旧存档默认启用、隐藏/回来、ducking、dispose 与零资产/零网络/零随机/包体 ≤2MB），post-launch 规则/音频专项 7/7，战斗表现专项 `test_battle_presentation` 39/39（连同完整性专项合计 45/45）。全屏舞台以真实 Chromium 在 390×844 / 844×390 / 1366×768 / 2048×1280 实测：overlay 四边与视口误差 0px、无横向溢出、无 token 越界、双控件可点、dispose 后 body class/overlay/HUD 全复原，截帧存 `work/shots/`。默认分享 demo 为首战 6.5 秒、Act 2 11.11 分钟、声望 100 结局 27.85 分钟，18/18 战斗脚本对账；390×844 冲锋中盘实测骑兵 x=240.2、民兵 157.6、老兵 161.2、弓兵 139.0。
...
- **战斗层必须占满视口。** `.battle-stage` 的 `fixed/inset:0/100dvh` 与不透明背景、`.stage-paper` 的 full-bleed（无宽高上限/阴影/外边距）、战斗中 `#app-overlay` 的 z-index 提升与「只留声/静 + 倍速两枚角落控件」是永久门禁项；不得恢复居中卡片、半透明遮罩或让地图从四周透出。`--field-w` 约束的是内容宽度不是卡片，改动需同时更新 `test_battle_fullscreen`。
- **战斗表现规范已冻结。** 唯一权威规范落地后只修 bug，不接新需求。以下不可回退：16 token 上限、每场两次震动且只动 world 层、三档重量的定格节奏、弓手四段满弓、2.2/1.0/0.85/0.6 兵种视觉速度、弓兵后排留守/射程弧/近身停射/最后受击、匪徒走入后排射手的近战距离后才命中、精英匪队披肩与狼尾旗、骑兵 2.2× 冲锋后硬停、部将放大 30% 且不排队、按目标实际坐标逐 token 朝向与背刺后转身、墨渍面积上限与渐隐、`emitBeat` 单一时钟。**同一选择器/keyframes 不得出现第二份更靠后的定义**——定格姿势、满弓与按档击退都曾因此静默失效，`test_battle_presentation` 现在按层叠顺序断言。
